...
class span_ion__comparator_fd_main2_dsn(DesignModule):
    ...
    def meet_spec(self, **params) -> List[Mapping[str,Any]]:
        ...
        vth_load = estimate_vth(is_nch=(not n_in), vgs=-vtest, vbs=0, db=db_dict['load'], lch=l_dict['load'])
        ...
        for vtail in vtail_vec:
            voutcm_min = vincm-vth_in if n_in else vstar_min
            voutcm_max = vdd-vstar_min if n_in else vincm-vth_in
            ...
            for voutcm in voutcm_vec:
                ...
                # Sweep load gate voltage
                vcmfb_min = voutcm+vth_load if n_in else vth_load+vstar_min
                vcmfb_max = vdd+vth_load-vstar_min if n_in else voutcm+vth_load
                vcmfb_vec = np.arange(vcmfb_min, vcmfb_max, res_vstep)
                for vcmfb in vcmfb_vec:
                    load_op = db_dict['load'].query(vgs=vcmfb-vb_load, vds=voutcm-vb_load, vbs=0)
                    for nf_in in nf_in_vec:
                        ibias = ibias_min * nf_in
                        ibranch = ibias/2

                        load_success, nf_load = verify_ratio(in_op['ibias'],
                                                             load_op['ibias'],
                                                             nf_in,
                                                             error_tol)
                        ...
                        ckt_half.add_transistor(load_op, 'out', 'gnd', 'gnd', fg=nf_load, neg_cap=False)
                        ...
                        for vgtail in vgtail_vec:
                            ...
                            viable_op = dict(nf_in=nf_in,
                                             ...
                                             cmfb_cload=load_op['cgg']*nf_load)
